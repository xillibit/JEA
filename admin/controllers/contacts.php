 ...
 * @subpackage  com_jea
 */
class JeaControllerContacts extends JControllerAdmin
{
    ...
    public function delete()
    {
      ...
      if ( $id > 0 ) {
        $query = "SELECT * FROM #__jea_contacts WHERE id={$id}";
        $db->setQuery($query);
        $contact = $db->loadObject();

        $query = "DELETE FROM #__jea_contacts WHERE id={$id}";
        $db->setQuery($query);
        $db->query();

        $app->enqueueMessage('Le contact nommé '.$contact->contactname.' '.$contact->lastname.' a bien été supprimé');
