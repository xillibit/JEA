 ...
 * @subpackage  com_jea
 */
class JeaModelDemandes extends JModelList
{
  /**
     * Constructor.
     *
    ...
    protected function getListQuery()
    {
        ...
        $query->select('d.*, c.contactname, c.lastname, c.adress, c.mail');

        $query->from('#__jea_demandes AS d');

        // Join contacts
        $query->join('LEFT', '#__jea_contacts AS c ON d.id_contact=c.id');
        ...
        // Join over the contact name
        $query->select('cont.contactname');
        $query->join('LEFT', $db->quoteName('#__jea_contacts_rel').' AS crel ON crel.property_id = p.id');
        $query->join('LEFT', $db->quoteName('#__jea_contacts').' AS cont ON cont.id = crel.contact_id'); 
        ...
        if ($search = $this->getState('filter.search')) {
            ...
            $search = '(p.ref LIKE ' . $search
                    ...
                    . ' OR p.id LIKE ' . $search
                    . ' OR cont.contactname LIKE ' .$search .')';
            $query->where($search);
        }
    ...
    public function getNbBiensEnRelation()
    {
      $db        = $this->getDbo();
      
      $query_items = $this->getListQuery();
      ...
      if ( !empty($items) )
      {
        ...
        $query->select('demande_id, COUNT(*) AS nombre');
  
        $query->from('#__jea_demandes_properties AS dp');
        
        $query->where("demande_id IN ({$items_array})");
        
        $query->group('demande_id');
        ...
        {
          $result[$object->demande_id] = $object->nombre;
        }
      }       
