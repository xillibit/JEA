...
class JeaControllerProperty extends JControllerForm
{
	...
	*/
	public function sendMailOwner()
	{
		...
		{
			$app->enqueueMessage('Vous devez entrer votre pr�nom', 'error');
			return $this->setRedirect($returnurl);
		}
		else if ( empty($telephone) )
		{
			$app->enqueueMessage('Vous devez entrer votre num�ro de t�l�phone', 'error');
			return $this->setRedirect($returnurl);
		}
		else if ( empty($email))
		{
		...
		// On r�cup�re les contacts associ�s � l'annonce
		...
		$mailer->setSubject('Bourse immobiliére demande d\'informations sur une annonce');
		$mailer->isHTML(true);
		$mailer->setBody(
			'Vous avez reçu une demande d\information depuis la Bourse immobiliére du site de la Communauté de Communes du Pays de Faverges (http://pays-de-faverges.com/).
		...
		// On envoie un mail au service économique
		$mailer->addRecipient('service.economique@mail.com');
		$mailer->setSubject('Bourse immobilière envoi d\'une demande');
		$mailer->isHTML(true);
		$mailer->setBody(
			'Une nouvelle demande d\'informations a été faite sur l\'annonce nommée <b>'.$result->title.'</b> portant la référence <b>'.$result->ref.'</b><br />
			<a href="'.JRoute::_('index.php?option=com_jea&view=property&id='. $slug, false).'" title="Lien vers l\'annonce du bien">Lien vers l\'annonce du bien en question</a><br />
			Détails des informations données par la personne demandeuse : <br />
			<ul>
			<li>Nom : '.$nom.'</li>
		...
		$app->enqueueMessage('Votre demande a bien été envoyée');
	...
	*/
	protected function insertDemande($nom, $prenom, $email, $telephone, $message, $property_id)
	{
		...
		// Modifier la base de donnée et la requête pour mettre nom du contact, prénom et date de demande
    ...
    public function sendMailContact()
    {
      ...
      // L'utilisateur doit compl�ter son nom, email, sujet et message sinon on le renvoie � la page pr�c�dente
      if ( empty($nom) && empty($email) && empty($telephone) && empty($sujet) && empty($message) ) {
        JFactory::getApplication()->enqueueMessage('Vous devez compléter tous les champs pour que le message soit envoy�', 'error');

        return $this->setRedirect($returnurl);
      }

      // Pr�paration pour l'envoi du mail
      $mailer = JFactory::getMailer();
      ...
      $mailer->setSubject('[Gestion immobiliére demande informations] '.$sujet);
      $mailer->isHTML(true);
      $mailer->setBody(
        'Detail du message: '.$message.' <br /><br /><b>Annonce
		d�pos�e le: </b>'.$date->format('d-m-Y').'<br /><br /><b>Coordonn�es
         du d�posant:</b> <br />'.$nom.' <br />'.$telephone.' <br />'.$email);
